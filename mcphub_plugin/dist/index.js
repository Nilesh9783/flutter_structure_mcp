...
export class PathUtils {
    static resolveSafePath(rawPath) {
...
export class SolutionGenerator {
    static attachSolutionAndPrompt(finding) {
    ...
    static generateSuggestedFix(finding) {
        const id = finding.id;
...
// Node:   logger.info("Application event");`;
    ...
    static generateClaudePrompt(finding, suggestedFix) {
...
Please inspect \`${finding.file}\`, apply the necessary changes preserving all existing behavior and APIs, and ensure the code compiles cleanly.`.trim();
    ...
    static generateMasterClaudePrompt(findings) {
            ...
            try {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    for (const pat of this.patterns) {
                        if (isClientConfig && pat.name === "Google API Key")
                            continue;
        ...
        if (fs.existsSync(pubspecPath)) {
            ...
            try {
                ...
                if (content.includes("flutter_bloc") || content.includes("bloc:"))
                    ...
                    detectedStateManagement = "GetX";
                else if (content.includes("provider:"))
                    detectedStateManagement = "Provider";
                else if (content.includes("flutter_riverpod") || content.includes("riverpod:"))
                    detectedStateManagement = "Riverpod";
                if (content.includes("go_router"))
                    detectedRouter = "GoRouter";
                else if (content.includes("auto_route"))
                    detectedRouter = "AutoRoute";
                if (content.includes("dio"))
                    detectedNetwork = "Dio";
                else if (content.includes("http:"))
                    detectedNetwork = "http";
                if (content.includes("hive"))
                    detectedDatabase = "Hive";
                else if (content.includes("isar"))
                    detectedDatabase = "Isar";
                else if (content.includes("sqflite"))
                    detectedDatabase = "Sqflite";
...
export class FullReportGenerator {
    static async generateAllReports(projectPath, meta, findings, score) {
    ...
    static generateMarkdown(meta, findings, score) {
    ...
    static generateInteractiveHtml(meta, findings, score) {
...
<body>
    ...
    static generatePdfHtml(meta, findings, score) {
...
// Helper Utilities
// ============================================================================
function listFilesRecursively(dir, allowedExtensions) {
    const results = [];
...
export default class NicFlutterStructureArchitect {
    name = "nic-flutter-structure-architect";
    ...
    version = "1.0.8";
    description = "Multi-technology codebase auditor for Flutter, Vue projects with interactive tabbed HTML reports.";
    tools = ALL_TOOLS;
    config = {
        scanLevel: "standard",
        generateReport: true,
    ...
    async executeTool(name, args = {}) {
        ...
        const rawInputPath = args.projectPath || args.path || this.config.defaultProjectPath;
